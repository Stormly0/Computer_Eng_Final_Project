...
#else
    LiquidCrystal_I2C CONTROL_LCD(CONTROL_LCD_ADDRESS, 20, 4); // System status LCD where you can see the modes and the current status of the system
#endif
...
// Update Environment speed 
const unsigned int Update_Environment_Speed = 1000; // Updates the temperature and humidity every 30 seconds on the system [ms]

// Lighting Parameters 
const unsigned int Max_Neo_Pixel_Brightness = 10; // Max Brightness of the NeoPixel [0 - 255]
const unsigned int Neo_Pixel_Delay_Between_Lights = 10; // Delay between each light in the NeoPixel [ms]
const unsigned int Max_LED_Brightness = 100; // Max Brightness of the LEDs [0 - 255]

// LCD parameters
const unsigned int LCD_Refresh_Rate = 2000; // How fast the LCD's updates [ms]
const unsigned int Max_LCD_Idle_Time = 10000; // Max time that the LCD can be idle before it turns off [ms]
...
Timer Update_Environment_Timer; 
...
void Set_Fan_Speed(unsigned int Speed){
    ...
    // Checks if it is in a virtual environment 
    #if VIRTUAL_ENVIRONMENT 
        // Calculates the Duty cycle for the pin (Simulated PWM)
        const double PWM_Frequency = 2024; 
...
void Set_LED(unsigned int LED_NUMBER, unsigned int BRIGHTNESS){
    ...
    // double Off_Duty_Cycle = PWM_Frequency - On_Duty_Cycle; // Calculates the off duty cycle of the PWM signal
        ...
        analogWrite(LED_NUMBER,BRIGHTNESS); 
    #endif
}
...
float Get_Temperature(){
    ...
    float Temperature = DHT_SENS.temperature; 
...
float Get_Humidity(){
   ...
   // float Voltage = analogRead(HUMIDITY_SENSOR);
    //Serial.println(Voltage); 
    float Humidity = DHT_SENS.humidity; 
...
void Display_Fan_Speed_Top(){
    // Checks if we are in the virtual environment or not 
        ...
        float Percentage = ((float)Fan_Speed/255) * 100; 

        CONTROL_LCD.print((int)Percentage);
...
void Display_Bottom_Basic(){
    ...
    // Checks whether we are in a virtual environment or a real environment 
        ...
        // Checks if the display has changed 
        if(Display_Changed){
            // Clears the bottom display 
            // for(int i = 0; i < 20; i++){
            //     CONTROL_LCD.setCursor(i,2); 
            //     CONTROL_LCD.print(" "); 

            //     CONTROL_LCD.setCursor(i,3); 
            //     CONTROL_LCD.print(" "); 
            // }
            CONTROL_LCD.clear(); // Clears the top display 
            Display_Changed = false; // Sets the display changed to false as we have just cleared the display
...
void Update_Environment_Data(){
    ...
    // Checks if the specific amount of time has passed 
    if(!Update_Environment_Timer.Check_Time_Millis(Update_Environment_Speed)){
        return; 
    }
...
void Update_System_On_Input(){
    // Checks if we are in a virtual environment or a real environment
        ...
        // Checks if the user wants to change the fan speed
        if(Active && (abs(Get_Set_Fan_Speed() - Fan_Speed) > 5)){
        ...
        // Checks if the user wants to change the set temperature
        if(Active && (abs(Get_Set_Temperature() - Set_Temperature)) > 5){
...
void Update_System_Components(){
    ...
    if(Display_Fan_Setting){
        Serial.println("DISPLAYING FAN SETTING"); 
        // Displays the fan setting 
        Display_Fan_Speed_Top();

        // Checks and waits a few seconds before updating the display again 
        ...
        Serial.println("DISPLAYING TEMPERATURE SETTING"); 
        ...
        //if(LCD_Refresh_Timer.Check_Time_Millis(LCD_Refresh_Rate)){
            Serial.println("DISPLAYING BASIC STATUS"); 
            Display_Top_Basic(); 
        //}
    }
